...
public class StripeFragment extends BasePaymentFragments {
    ...
    private void getAllCards() {
        ...
        map.put(Const.Params.PAYMENT_GATEWAY_ID, Const.Payment.STRIPE);
    ...
    private void openAddCardDialog() {
        ...
        addCardDialog.findViewById(R.id.btnDialogAlertLeft).setOnClickListener(view -> addCardDialog.dismiss());

        addCardDialog.setOnDismissListener(dialog -> paymentActivity.getWindow().setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_STATE_ALWAYS_HIDDEN));
        ...
        call.enqueue(new Callback<PaymentResponse>() {
            ...
            public void onResponse(@NonNull Call<PaymentResponse> call, @NonNull Response<PaymentResponse> response) {
                if (response.isSuccessful() && response.body().isSuccess()) {
                    PaymentMethodCreateParams createPaymentParams = PaymentMethodCreateParams.create(stripeCard.getPaymentMethodCard(), new PaymentMethod.BillingDetails.Builder().setName(etCardHolderName.getText().toString().trim()).setEmail(preferenceHelper.getEmail()).setPhone(preferenceHelper.getPhoneCountyCode() + preferenceHelper.getPhoneNumber()).build());
