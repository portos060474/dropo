...
import com.dropo.interfaces.CircularProgressViewListener;
...
public class CustomCircularProgressView extends View {
    ...
    public CustomCircularProgressView(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void initAttributes(AttributeSet attrs, int defStyle) {
        final TypedArray a = getContext().obtainStyledAttributes(attrs, R.styleable.CustomCircularProgressView, defStyle, 0);

        Resources resources = getResources();

        // Initialize attributes from styleable attributes
        currentProgress = a.getFloat(R.styleable.CustomCircularProgressView_cpv_progress, resources.getInteger(R.integer.cpv_default_progress));
        maxProgress = a.getFloat(R.styleable.CustomCircularProgressView_cpv_maxProgress, resources.getInteger(R.integer.cpv_default_max_progress));
        thickness = a.getDimensionPixelSize(R.styleable.CustomCircularProgressView_cpv_thickness, resources.getDimensionPixelSize(R.dimen.cpv_default_thickness));
        isIndeterminate = a.getBoolean(R.styleable.CustomCircularProgressView_cpv_indeterminate, resources.getBoolean(R.bool.cpv_default_is_indeterminate));
        autostartAnimation = a.getBoolean(R.styleable.CustomCircularProgressView_cpv_animAutostart, resources.getBoolean(R.bool.cpv_default_anim_autostart));
        initialStartAngle = a.getFloat(R.styleable.CustomCircularProgressView_cpv_startAngle, resources.getInteger(R.integer.cpv_default_start_angle));
        ...
        if (a.hasValue(R.styleable.CustomCircularProgressView_cpv_color)) {
            color = a.getColor(R.styleable.CustomCircularProgressView_cpv_color, ResourcesCompat.getColor(resources, R.color.cpv_default_color, null));
        }
        ...
        // If using native accentColor (SDK >21)
            ...
            color = t.getColor(0, ResourcesCompat.getColor(resources, R.color.cpv_default_color, null));
        }

        animDuration = a.getInteger(R.styleable.CustomCircularProgressView_cpv_animDuration, resources.getInteger(R.integer.cpv_default_anim_duration));
        animSwoopDuration = a.getInteger(R.styleable.CustomCircularProgressView_cpv_animSwoopDuration, resources.getInteger(R.integer.cpv_default_anim_swoop_duration));
        animSyncDuration = a.getInteger(R.styleable.CustomCircularProgressView_cpv_animSyncDuration, resources.getInteger(R.integer.cpv_default_anim_sync_duration));
        animSteps = a.getInteger(R.styleable.CustomCircularProgressView_cpv_animSteps, resources.getInteger(R.integer.cpv_default_anim_steps));
        a.recycle();
    }
    ...
    private void updateBounds() {
        ...
        bounds.set((float) paddingLeft + thickness, (float) paddingTop + thickness, (float) size - paddingLeft - thickness, (float) size - paddingTop - thickness);
    }
    ...
    private AnimatorSet createIndeterminateAnimator(float step) {
        ...
        rotateAnimator1.addUpdateListener(animation -> indeterminateRotateOffset = (Float) animation.getAnimatedValue());
        ...
        rotateAnimator2.addUpdateListener(animation -> indeterminateRotateOffset = (Float) animation.getAnimatedValue());
