...
public class OrderProductAdapter extends SectionedRecyclerViewAdapter<RecyclerView.ViewHolder> {
    private final ArrayList<CartProducts> cartProductList;
    private final OrderPreparedFragment orderPreparedFragment;
    private final ParseContent parseContent;

    public OrderProductAdapter(OrderPreparedFragment orderPreparedFragment, ArrayList<CartProducts> cartProductList) {
        this.orderPreparedFragment = orderPreparedFragment;
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, final int section, final int relativePosition, int absolutePosition) {
        ...
        final String price = OrderEdit.getInstance().getOrderCurrency() + parseContent.decimalTwoDigitFormat.format(cartProductItems.getTotalItemAndSpecificationPrice());
        ...
        if (orderPreparedFragment.getContext() != null) {
            Utils.setLeftBackgroundRtlView(orderPreparedFragment.getContext(), cartHolder.btnIncrease);
            Utils.setRightBackgroundRtlView(orderPreparedFragment.getContext(), cartHolder.btnDecrease);
        }

        cartHolder.btnIncrease.setOnClickListener(view -> orderPreparedFragment.increaseItemQuantity(cartProductList.get(section).getItems().get(relativePosition)));
        cartHolder.btnDecrease.setOnClickListener(view -> orderPreparedFragment.decreaseItemQuantity(cartProductList.get(section).getItems().get(relativePosition)));
        cartHolder.tvRemoveCartItem.setOnClickListener(view -> orderPreparedFragment.removeItem(section, relativePosition));
        cartHolder.itemView.setOnClickListener(v -> {
            ...
            orderPreparedFragment.goToProductSpecificationActivity(section, relativePosition, cartProductList.get(section).getItems().get(relativePosition), productDetail);
        });
    }
