...
public abstract class ServerDialog extends BottomSheetDialog implements View.OnClickListener {
    ...
    private final CustomEditText etServerUrl;
    private final CustomImageView btnClose;
    private final CustomButton btnOk;
        ...
        if (id == R.id.btnClose) {
            ...
            if (checkedRadioButtonId == R.id.rbServer1) {
                ...
                if (TextUtils.isEmpty(etServerUrl.getText()) || !Patterns.WEB_URL.matcher(etServerUrl.getText()).matches()) {
                    Utilities.showToast(activity, activity.getString(R.string.msg_please_enter_url));
                    return;
                } else {
    ...
    private void goWithOk() {
        if (!ServerConfig.BASE_URL.equals(PreferenceHelper.getPreferenceHelper(activity).getBaseUrl())) {
            onOkClicked();
        }
        dismiss();
    }
    ...
    private void goWithOtherURL() {
        ...
        Call<AppSettingResponse> call = new ApiClient().changeApiBaseUrl(ServerConfig.BASE_URL).create(ApiInterface.class).getAppSettingDetail(map);
            ...
            public void onResponse(@NonNull Call<AppSettingResponse> call, @NonNull Response<AppSettingResponse> response) {
                ...
                if (ParseContent.getInstance().isSuccessful(response)) {
                    ...
                    if (response.body() != null) {
                        ParseContent.getInstance().showErrorMessage(activity, 0, response.body().getStatusPhrase());
                    }
                }
            }
