...
public abstract class DialogVerifyPayment extends BottomSheetDialog implements View.OnClickListener, TextView.OnEditorActionListener {
    ...
    private final CustomButton btnDialogAlertRight;
    private final CustomFontTextViewTitle tvDialogTitle;
    private final CustomTextView tvDialogMessage;
    ...
    public DialogVerifyPayment(Context context, String requiredParam, String reference) {
        ...
        switch (requiredParam) {
            case Constant.VerificationParam.SEND_OTP: {
            ...
            break;
            case Constant.VerificationParam.SEND_PIN: {
            ...
            break;
            case Constant.VerificationParam.SEND_ADDRESS: {
            ...
            break;
            case Constant.VerificationParam.SEND_BIRTHDATE: {
            ...
            break;
            case Constant.VerificationParam.SEND_PHONE: {
    ...
    private void doWithEnable(EditText editText) {
        if (!TextUtils.isEmpty(etPin.getText().toString().trim())) {
            ...
            map.put(Constant.REFERENCE, reference);
            map.put(Constant.REQUIRED_PARAM, requiredParam);
            switch (requiredParam) {
                case Constant.VerificationParam.SEND_OTP: {
                    map.put(Constant.OTP, input);
                }
                break;
                case Constant.VerificationParam.SEND_PIN: {
                    map.put(Constant.PIN, input);
                }
                break;
                case Constant.VerificationParam.SEND_ADDRESS: {
                    map.put(Constant.ADDRESS, input);
                }
                break;
                case Constant.VerificationParam.SEND_BIRTHDATE: {
                    map.put(Constant.BIRTHDAY, input);
                }
                break;
                case Constant.VerificationParam.SEND_PHONE: {
                    map.put(Constant.PHONE, input);
                }
            }
            doWithEnable(map);
        } else {
            Utilities.showToast(getContext(), getContext().getString(R.string.msg_plz_enter_valid_field));
        }
    }
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.btnDialogAlertRight) {
            doWithEnable(etPin);
        }
    }
