...
class HelpVC: BaseVC {
    ...
    func setLocalization() {
        ...
        btnCall.setImage(UIImage.init(named: "callIcon")?.imageWithColor(color: UIColor.themeColor), for: .normal)
        btnMail.setImage(UIImage.init(named: "mailIcon")?.imageWithColor(color: UIColor.themeColor), for: .normal)
    ...
    @IBAction func onClickBtnPrivacy(_ sender: Any) {
        ...
        guard let url = URL(string:WebService.USER_PANEL_URL + "legal/user-privacy-policy") else {
            return
        }
