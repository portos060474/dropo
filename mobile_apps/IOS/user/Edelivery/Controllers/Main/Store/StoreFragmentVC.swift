...
class StoreFragmentVC: BaseVC {
    ...
    var finalFilteredArray:[StoreItem] = []
    var originalArrStoreList:[StoreItem]? = nil
    var filteredArrStoreList:[StoreItem]? = nil
    ...
    func timerSetup() {
        ...
        timerForAdd = Timer.scheduledTimer(timeInterval: 3.0, target: self, selector: #selector(StoreFragmentVC.handleAddTimer), userInfo: nil, repeats: true)
    }
    ...
    @objc func onClickFilter() {
        if viewForMap.isHidden {
            ...
            Utility.showToast(message: "MSG_CLOSE_MAP_FIRST".localized)
        }
    }
    ...
    func onClickMap() {
        ...
        if viewForFilter.isHidden {
            ...
            Utility.showToast(message: "MSG_CLOSE_FILTER_FIRST".localized)
        }
    }
...
extension StoreFragmentVC:UITableViewDataSource, UISearchBarDelegate,UITableViewDelegate,GMSMapViewDelegate {
    func mapView(_ mapView: GMSMapView, didTapInfoWindowOf marker: GMSMarker) {
        self.selectedStoreItem = marker.userData as? StoreItem
        selectedFrom = .fromMapView
        self.goToProductVC()
    }
        ...
        else
        {
            ...
            if btnByItem.isSelected {
                ...
                for storesItem:StoreItem in tempArray {
                    ...
                    {
                        if tagName.tag.lowercased() == mySearchText.lowercased() {
                            tempFiltereArray.append(storesItem)
                        }
                    }
                }
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if (storeId.isEmpty()) {
            ...
            selectedFrom = .fromCollectionView
            let productVc : ProductVC = ProductVC.init(nibName: "Product", bundle: nil)
...
// MARK: - TransitionAnimating/TransitionDelegate
extension StoreFragmentVC:UICollectionViewDelegate,UICollectionViewDataSource, UICollectionViewDelegateFlowLayout,UIScrollViewDelegate {
...
extension StoreFragmentVC:HTagViewDelegate,HTagViewDataSource {
