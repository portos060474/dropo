...
public class HTag: UIView {
    ...
    func setupCancelButton() {
        cancelButton.setImage(UIImage(named: "canceRequestIcon", in: Bundle(for: self.classForCoder), compatibleWith: nil)?.withRenderingMode(.alwaysTemplate), for: UIControl.State())
    ...
    func updateTitlesColorsAndFontsDueToSelection() {
        ...
        attributes[convertFromNSAttributedStringKey(NSAttributedString.Key.font)] = tagFont
        attributes[convertFromNSAttributedStringKey(NSAttributedString.Key.foregroundColor)] = textColor
        button.setAttributedTitle(NSAttributedString(string: tagTitle, attributes: convertToOptionalNSAttributedStringKeyDictionary(attributes)), for: .normal)
        if tagType == .cancel {
