...
open class SkyFloatingLabelTextField: UITextField {
    ...
    @IBInspectable dynamic open var placeholderColor: UIColor = UIColor.themeLightTextColor {
        didSet {
            updatePlaceholder()
        }
    }
    ...
    fileprivate func updatePlaceholder() {
        if let placeholder = placeholder, let font = placeholderFont ?? font {
            attributedPlaceholder = NSAttributedString(
                string: placeholder,
                attributes: convertToOptionalNSAttributedStringKeyDictionary([convertFromNSAttributedStringKey(NSAttributedString.Key.foregroundColor): placeholderColor, convertFromNSAttributedStringKey(NSAttributedString.Key.font): font])
            )
        }
    }
    /// A UIColor value that determines the text color of the title label when in the normal state
    @IBInspectable dynamic open var titleColor: UIColor = .themeLightTextColor {
        didSet {
            updateTitleColor()
        }
    }
    /// A UIColor value that determines the color of the bottom line when in the normal state
    @IBInspectable dynamic open var lineColor: UIColor = .themeLightLineColor {
        didSet {
            updateLineView()
        }
    }
    ...
    @IBInspectable dynamic open var selectedTitleColor: UIColor = .themeLightTextColor {
        didSet {
            updateTitleColor()
        }
    }
    /// A UIColor value that determines the color of the line in a selected state
    @IBInspectable dynamic open var selectedLineColor: UIColor = .themeTextColor {
        didSet {
            updateLineView()
        }
    }
    ...
    fileprivate func createTitleLabel() {
        ...
        titleLabel.font = FontHelper.textSmall()
    ...
    open func updateTitleHide(_ animated: Bool = false, completion: ((_ completed: Bool) -> Void)? = nil) {
        let alpha: CGFloat = isTitleVisible() ? 1.0 : 0.0
        let frame: CGRect = titleLabelBounds(bounds, editing: isTitleVisible())
        let updateBlock = { () -> Void in
            self.titleLabel.alpha = alpha
            self.titleLabel.frame = frame
        }
    ...
    open func titleLabelBounds(_ bounds: CGRect, editing: Bool) -> CGRect {
        if editing {
...
class CustomTextfield : SkyFloatingLabelTextField
{
    ...
    @IBInspectable dynamic open var leftPadding: CGFloat = 0 {
        didSet {
            
        }
    }
    
    @IBInspectable dynamic open var rightPadding: CGFloat = 0 {
        didSet {
    ...
    override open func placeholderRect(forBounds bounds: CGRect) -> CGRect {
        ...
        let rect = CGRect(
            x: isLTRLanguage ? leftPadding : rightPadding,
            y: titleHeight(),
    ...
    override func titleLabelRectForBounds(_ bounds: CGRect, editing: Bool) -> CGRect {
        ...
        if editing {
            return CGRect(x: isLTRLanguage ? leftPadding : rightPadding, y: 5, width: bounds.size.width, height: titleHeight())
        }
        
        return CGRect(x: isLTRLanguage ? leftPadding : rightPadding, y: titleHeight(), width: bounds.size.width, height: titleHeight())
    }*/
    ...
    func setup() {
        ...
        self.tintColor = .themeIconTintColor
    }
