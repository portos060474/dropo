...
//    func checkPhoneValidation() -> Bool {
//        if self.isEmpty() {
//            return false
//        } else {
//            let phoneRex = "\+[0-9]+[\- \.]*)?(\([0-9]+\)[\- \.]*)?([0-9][0-9\- \.]+[0-9]"
//            let phoneTest:NSPredicate = NSPredicate(format:"SELF MATCHES %@", phoneRex)
    ...
    func isValidMobileNumber() -> (Bool, String) {
        if self.isEmpty() {
            ...
            if self.count < min || self.count > max {
                return (false, "msg_enter_valid_phone_number".localized)
            } else {
                return (true, "")
            }
        }
