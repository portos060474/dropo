...
class MessageHandler:NSObject{
    ...
    func readMessage(id:String)
    {
        ...
        DBProvider.Instance.messageRef.child(MessageHandler.Order_ID).child("\(MessageHandler.chatType)").child(id).child(CONSTANT.MESSAGES.STATUS).setValue(true)
    }
    ...
    func observeMessage()
    {
       ...
       print("chatType : \(MessageHandler.chatType)")
        ...
        DBProvider.Instance.messageRef.child(MessageHandler.Order_ID).child("\(MessageHandler.chatType)").observe(.childAdded){(snapShot:DataSnapshot) in
            if snapShot.exists(){
                if let data = snapShot.value as? [String:Any]
                {
                    print("selectedOrderId : \(MessageHandler.Order_ID)")
    ...
    {
        Utility.showLoading()
        DBProvider.Instance.messageRef.child(MessageHandler.Order_ID).child("\(MessageHandler.chatType)").observe(.childChanged){(snapShot:DataSnapshot) in
   ...
   /* func continueObserveMessage() {
    ...
    func sendMessage(text:String,time:String){
        ...
        let key = (DBProvider.Instance.messageRef.child(MessageHandler.Order_ID).childByAutoId().key)
...
//                                           CONSTANT.MESSAGES.TYPE:CONSTANT.TYPE_USER,
                                            CONSTANT.MESSAGES.SENDER_TYPE:CONSTANT.CHAT_DELIVERYMAN_TYPE,
...
//        DBProvider.Instance.messageRef.child(CurrentBooking.shared.selectedOrderId!).child(key!).setValue(data)
        DBProvider.Instance.messageRef.child(MessageHandler.Order_ID).child("\(MessageHandler.chatType)").child(key!).setValue(data)
 
    }
    
    func removeObserver()
    {
        DBProvider.Instance.messageRef.child(MessageHandler.Order_ID).child(CONSTANT.DBPROVIDER.MESSAGES).removeAllObservers()
    }
