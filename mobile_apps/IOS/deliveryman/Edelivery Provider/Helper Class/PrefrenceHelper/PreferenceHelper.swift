...
class PreferenceHelper: NSObject {
    ...
    private let KEY_IS_ACTIVE_FOR_JOB = "is_active_for_job";
    ...
    private let KEY_IS_ONLINE = "is_online";
    ...
    private let KEY_IS_PROVIDER_DOCUMENT_UPLOADED = "is_provider_document_uploaded";
    private let KEY_IS_EMAIL_VERIFIED = "is_email_verified";
    private let KEY_IS_PHONE_NUMBER_VERIFIED = "is_phone_number_verified";
    private let KEY_UNIQUE_ID = "uninque_id";
    /*check Admin Flags*/
    private let KEY_IS_PHONE_NUMBER_VERIFICATION = "is_sms_verification";
    private let KEY_IS_EMAIL_VERIFICATION = "is_mail_verification";
    ...
    private let KEY_SELECTED_VEHICLE_ID = "selected_vehicle_id"
    private let KEY_IS_ALL_VEHICLE_DOCUMENT_UPLOADED = "is_all_vehicle_document_uploaded"
    private let KEY_REQUEST_ALERT = "key_request_alert";
    private let KEY_PUSH_SOUND = "key_push_sound";
    private let KEY_DARK_MODE_ON = "key_dark_mode_on"
    ...
    private let IS_CUSTOM_LOCATION = "is_custom_location"
    ...
    func setIsRequesetAlert(_ isRequestAlert:Bool) {
        ph.set(isRequestAlert, forKey: KEY_REQUEST_ALERT);
        ph.synchronize();
    }
    
    func getIsRequesetAlert() -> Bool {
        return (ph.value(forKey: KEY_REQUEST_ALERT) as? Bool) ?? true
    }
    
    func setIsPushSoundOn(_ isPushSoundOn:Bool) {
        ph.set(isPushSoundOn, forKey: KEY_PUSH_SOUND);
        ph.synchronize();
    }
    
    func getIsPushSoundOn() -> Bool {
        return (ph.value(forKey: KEY_PUSH_SOUND) as? Bool) ?? true
    }
    
    func setIsAllVehicleDocumentUploaded(_ isAllVehicleDocumentUploaded:Bool) {
        ph.set(isAllVehicleDocumentUploaded, forKey: KEY_IS_ALL_VEHICLE_DOCUMENT_UPLOADED);
        ph.synchronize();
    }
    
    func getIsAllVehicleDocumentUploaded() -> Bool {
        return (ph.value(forKey: KEY_IS_ALL_VEHICLE_DOCUMENT_UPLOADED) as? Bool) ?? false
    }

    func setSelectedVehicleId(_ vehicleId:String) {
        ph.set(vehicleId, forKey: KEY_SELECTED_VEHICLE_ID);
        ph.synchronize();
    }
    
    func getSelectedVehicleId() -> String {
        return (ph.value(forKey: KEY_SELECTED_VEHICLE_ID) as? String) ?? ""
    }
    
    func setCityId(_ cityId:String) {
        ph.set(cityId, forKey: KEY_CITY_ID);
        ph.synchronize();
    }
    
    func getCityId() -> String {
        return (ph.value(forKey: KEY_CITY_ID) as? String) ?? ""
    }
    ...
    func setUniqueId(_ id:Int) {
        ph.set(id, forKey: KEY_UNIQUE_ID);
        ph.synchronize();
    }
    
    func getUniqueId() -> Int {
        return (ph.value(forKey: KEY_UNIQUE_ID) as? Int) ?? 0
    }
    ...
    func getWalletCurrencyCode() -> String {
        return (ph.value(forKey: KEY_WALLET_CURRENCY_CODE) as? String) ?? ""
    }
    ...
    func getIsDarkModeOn() -> Bool {
        return (ph.value(forKey: KEY_DARK_MODE_ON) as? Bool) ?? false
    }
    
    func setIsDarkModeOn(_ isDarkModeOn:Bool) {
        ph.set(isDarkModeOn, forKey: KEY_DARK_MODE_ON);
        ph.synchronize();
    }
    ...
    func setIsPhoneNumberVerification(_ isSmsVerification:Bool) {
        ph.set(isSmsVerification, forKey: KEY_IS_PHONE_NUMBER_VERIFICATION);
        ph.synchronize();
    }
    ...
    func setIsEmailVerification(_ isMailVerification:Bool) {
        ph.set(isMailVerification, forKey: KEY_IS_EMAIL_VERIFICATION);
        ph.synchronize();
    }
    ...
    func setIsProviderDocumentUploaded(_ isProviderDocumentUploaded:Bool) {
        ph.set(isProviderDocumentUploaded, forKey: KEY_IS_PROVIDER_DOCUMENT_UPLOADED);
        ph.synchronize();
    }
    
    func getIsProviderDocumentUploaded() -> Bool {
        return (ph.value(forKey: KEY_IS_PROVIDER_DOCUMENT_UPLOADED) as? Bool) ?? false
    }
    
    func setIsRefferal(_ isRefferal:Bool) {
        ph.set(isRefferal, forKey: KEY_IS_REFERRAL);
        ph.synchronize();
    }
    
    func getIsRefferal() -> Bool {
        return (ph.value(forKey: KEY_IS_REFERRAL) as? Bool) ?? false
    }
    ...
    func setIsShowOptionalFieldInRegister(_ show:Bool) {
        ph.set(show, forKey: KEY_IS_SHOW_OPTIONAL_FIELD_IN_REGISTER);
        ph.synchronize();
    }
...
// MARK: Preference Provider Getter Setters
    ...
    func setIsOnlineOffline(_ isOnline: Bool) {
        ph.set(isOnline, forKey: KEY_IS_ONLINE);
        ph.synchronize();
    }
    
    func getIsOnlineOffline() -> Bool {
        return (ph.value(forKey: KEY_IS_ONLINE) as? Bool) ?? false
    }
    
    func setIsActiveJob(_ isActiveJob: Bool) {
        ph.set(isActiveJob, forKey: KEY_IS_ACTIVE_FOR_JOB);
        ph.synchronize();
    }
    
    func getIsActiveJob() -> Bool {
        return (ph.value(forKey: KEY_IS_ACTIVE_FOR_JOB) as? Bool) ?? false
    }
    ...
    func setIsLoginByEmail(_ isloginByEmail:Bool) {
        ph.set(isloginByEmail, forKey: KEY_IS_LOGIN_BY_EMAIL);
        ph.synchronize();
    }
    ...
    func setIsLoginByPhone(_ isloginByphone:Bool) {
        ph.set(isloginByphone, forKey: KEY_IS_LOGIN_BY_PHONE);
        ph.synchronize();
    }
    ...
    func setIsProviderApprove(_ isApprove: Bool) {
        ph.set(isApprove, forKey: KEY_IS_APPROVE);
        ph.synchronize();
    }
    
    func getIsProviderApprove() -> Bool {
    ...
    func setIsCustomLocation(_ bool:Bool) {
        ph.set(bool, forKey: IS_CUSTOM_LOCATION);
        ph.synchronize();
    }
    
    func getIsCustomLocation() -> Bool {
        return ph.value(forKey: IS_CUSTOM_LOCATION) as? Bool ?? false
    }
