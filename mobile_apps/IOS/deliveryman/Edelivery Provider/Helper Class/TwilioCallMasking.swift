...
class TwilioCallMasking {
    ...
    func wsTwilloCallMasking(id: String, type: String) {
        ...
        let dictParam:[String:Any] = [PARAMS.SERVER_TOKEN : preferenceHelper.getSessionToken(),
             PARAMS.PROVIDER_ID : preferenceHelper.getUserId(),
             PARAMS.ORDER_ID : id, PARAMS.call_to_usertype : type]
        
        let afn:AlamofireHelper = AlamofireHelper.init()
        afn.getResponseFromURL(url: WebService.WS_TWILIO_VOICE_CALL_PROVIDER, methodName: AlamofireHelper.POST_METHOD, paramData: dictParam) { [weak self] (response,error) -> (Void) in
            Utility.hideLoading()
