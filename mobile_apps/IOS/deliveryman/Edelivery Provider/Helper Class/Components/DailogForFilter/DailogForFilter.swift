...
class DailogForFilter: CustomDialog,UITextFieldDelegate  {
    ...
    func setupLocalization() {
        ...
        viewForTo.backgroundColor = .themeViewLightBackgroundColor
    
        viewForFrom.backgroundColor = .themeViewLightBackgroundColor
        alertView.backgroundColor = .themeAlertViewBackgroundColor
        self.alertView.updateConstraintsIfNeeded()
        self.alertView.roundCorner(corners: [.topRight , .topLeft], withRadius: 20)
        self.backgroundColor = UIColor.themeOverlayColor
        ImageForFrom.image = UIImage.init(named: "calender_white")?.imageWithColor(color: .themeIconTintColor)
        ImageForTo.image = UIImage.init(named: "calender_white")?.imageWithColor(color: .themeIconTintColor)
    }
