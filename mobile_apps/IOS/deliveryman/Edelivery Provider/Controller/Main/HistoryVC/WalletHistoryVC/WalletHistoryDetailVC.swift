...
//MARK:- Set localized layout
    ...
    func setWalletData() {
        if let walletRequestData = walletDetail {
            ...
            lblWalletREquestDate.text = Utility.stringToString(strDate: walletRequestData.createdAt, fromFormat: DATE_CONSTANT.DATE_TIME_FORMAT_WEB, toFormat: DATE_CONSTANT.DATE_TIME_FORMAT_AM_PM)
            ...
            if (walletRequestData.currentRate == 1.0) {
                print("viewForCurrentRate.isHidden = true")
                viewForCurrentRate.isHidden = true
            } else {
                viewForCurrentRate.isHidden = false
                lblCurrentRateValue.text =  "1 " + walletRequestData.fromCurrencyCode + " (" + String(format:"%.4f",[walletRequestData.currentRate]) + walletRequestData.toCurrencyCode + ")"
            }
        }
    }
}
